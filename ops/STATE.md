...
## ★ U-036 e2e 1차 실판 — 비용 효율 수술(2026-07-21 이현준-6)
- **1차 실판(게임 '턴 기반 육성 경영')이 $15 캡에서 quota_halt로 정상 정지 — 무한루프·표류·재배달 0(안정성 전 항목 통과), 산출물은 설계문서까지(게임 코드 착수 전).** 실측 규명 4건과 수리:
  - **크레딧 누수(1500→1852)**: quota 게이트가 리더 세그먼트 경계에만 있어 회의 전원발언(14분 23턴)이 초과분 태움 → **run_turn(모든 봇 wake 단일 관문) 진입 게이트**. 초과를 턴 1개로 한정.
  - **계획이 예산 52%($9.6/165턴)**: 팀 11개 상이직군 전원 합류(dedup 못 뚫음) → **자동 팀 상한 ORGANT_TEAM_CAP(기본 6, role_fit 상위·env 조정)**. 회의도 6명 기반으로 절감.
  - **디자이너가 프/백 코드 없이 로직 점유**: 근본=로드맵 첫 단계가 'V0 페이퍼 검증'(문서라 기획/디자이너 점유) → **마일스톤 회의 골격에 '첫 주기=브라우저에서 도는 실행물, 페이퍼 단계 금지' 명문화**.
  - **하이쿠인데 과다**: 단가 아님 — 턴 수(219)·회의 input이 뿌리(resume 델타는 이미 적용). 팀 상한이 주 해결.
- **요금 정합**: 정책 가정('작은 프로젝트 500~800cr')의 2.3배가 실측 — 가격이 아니라 효율을 가정치로 되돌리는 게 답(사용자 확정). 위 수리가 그 방향.
- **보류(등재)**: 백로그 도메인 소유 게이트(디자이너가 로직 백로그 self-claim 차단)는 role_fit 시소러스가 희소(게임 안건에 게임기획자만 >0)해 정밀 판정 불가 → role_fit 개선 후 재검토. 현재는 팀 상한+로드맵 코드화로 우회.
- 라이브 반영: 러너 재시작 필요(브레인 전량). @hj 스타터(1500cr)·enforce=1 유지.
