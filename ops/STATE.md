...
```
claude-company  1835bfa  ← 브레인 — 2026-07-14 ★주기완수=조건+하위ST전부완료 게이트(wrapup_done — 조건 4/4여도
...
murmur  d3fefb2   ← HEAD — atelier 치유 캐시 굶김 수리(d3fefb2) · B-7 역링크(a72e80e) · B-4 응답기(72357ce) — 프론트 빌드됨
                          장부 그대로 — 게이트가 하위완료 강제하므로) · ★목표·로스터 last_task 폴백(마감 후에도 판 정체 표시)
                          · ★피드 회의 표면화(회의·표결 행 백로그 귀속 제외 — 피벗 심의 실종 수리)
                          + 선점=작업중 즉시 표시(장부 우선, '▶ 작업중' 칩) — 3ee8e9f, 빌드 배포됨
