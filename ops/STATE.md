...
- ch94/P-045에서 백로그 5건과 실물 산출물은 끝났지만 조건 `met:0/2`로 남아 재픽 순환하던 비대칭을 닫음. **백로그가 전부 소진되면 SYS가 마일스톤 검증을 구조적으로 촉발**한다.
...
- 07-22 계약대로 백로그가 소진된 SubTask는 검증 게이트 없이 구조적으로 닫은 뒤 마일스톤만 실증한다. 성공은 `iter_verify → wrapup_done → done`, 실패는 조건별 기본 2회(`ORGANT_MILESTONE_VERIFY_CAP`) 뒤 `blocked_pending` 사람 확인으로 파킹한다. 시도 횟수는 Criterion 체크포인트에 동승해 재시작으로 상한이 리셋되지 않는다.
- 회귀: 성공(실제 파일 셸 검증→SubTask/MS done)·실패(2회→거짓 완료 없이 blocked_pending) 관통 테스트 추가. 새 판 라이브 실측(`milestone_auto_verify`·`ms_done`, `continue_incomplete` 순환 소멸)은 러너 반영 후 남음.
