...
- **★U-039 4차 실판 정산(ch84, 07-21) — 완주 실패의 비용 구조 확정**: 총 1,963cr(본판 1,530 캡 정지 + 재개 공회전 433)에 백로그 완주 1/34건. 분해 = 계획 912cr/137턴(60%) · 실작업 617cr/32턴(완주 0 — 스키마 작업 중 캡) · 재개 공회전 433cr(완료 단위 재검증 루프 — report_iter done-단위 차단으로 수리). 수확: 중지→재개 경로 검증(상태 라벨·복구), 구조 반려 게이트 2종(서브태스크 영역 중복·마일스톤 과정서술) 착지, 표결 전원·2층 실작동. **비용 근본 = 회의 지배(60%) + 협의 파생 백로그(34건 중 '-협의·-수치' 꼬리)** → 흡수 코칭 착지(협의=부모 백로그 완료 조건으로, 별도 항목 금지). 완주 전망 1,200~1,300. **미결 레버(사용자 결정 대기): 팀 문턱 step 2→3(env, -150~250) · 회의 병합 subtask+backlog(존재론, -120~180). 새 판은 최적화 결정 후.**
