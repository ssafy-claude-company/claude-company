...
- 라이브 반영: 러너 재시작 필요(브레인 전량). @hj 스타터(1500cr)·enforce=1 유지. 다음 실판용 @hj 크레딧 리셋(사용자가 이현준-6에 지시했으나 AUP 차단으로 미처리)은 라이브 DB 액션 — 사용자 확인 후 별도 처리.
...
- **라이브 반영 남음**: 러너 재시작(브레인 전량) + murmur-web 재시작(bridge C) — 세션 규율상 사용자 승인 대기. 프론트(dist)는 착지 즉시 서빙.
