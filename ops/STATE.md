...
- **다음 수리(미착수)**: ①SYS 자동 검증이 GOAL 잠금 조건에 대해서는 **비준된 명령**을 실행하도록
  일치시키거나 ②잠금이 SYS receipt를 동등하게 인정하도록 — 둘 중 하나. 그리고 재개방이 **연속 N회면
  파킹**(무한 순환 차단)이 필요하다. 판은 비용 때문에 정지시켰다.
- **부수 관측**: e2e가 열린 뒤에도 다른 봇들이 산출물을 계속 수정해 검증이 무효화되는 구간이 있었다
  (초기 2회). 검증 구간의 산출물 쓰기 잠금이 별도 개선점.
