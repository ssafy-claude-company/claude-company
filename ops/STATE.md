...
- 07-22 계약대로 백로그가 소진된 SubTask는 검증 게이트 없이 구조적으로 닫은 뒤 마일스톤만 실증한다. 단 **선행 필요 `blocked` 백로그는 원본을 보존**한다. 조건 검증이 성공해도 SubTask/MS를 닫지 않고 보충 백로그 회의를 열어 선행 일감을 추가하며, 선행 완료 뒤 원 blocked 작업까지 재개·완료돼야 `wrapup_done → done`이다(최대 구현 게이트). 검증 실패는 조건별 기본 2회(`ORGANT_MILESTONE_VERIFY_CAP`) 뒤 `blocked_pending` 사람 확인으로 파킹한다. 시도 횟수는 Criterion 체크포인트에 동승해 재시작으로 상한이 리셋되지 않는다.
- 회귀: 성공(실제 파일 셸 검증→SubTask/MS done)·실패(2회→거짓 완료 없이 blocked_pending)·**검증 성공+blocked 잔존→MS open/원본 보존/보충회의** 관통 테스트 추가. 새 판 라이브 실측(`milestone_auto_verify`·`ms_done`, `continue_incomplete` 순환 소멸)은 러너 반영 후 남음.
