...
def test_B20_peers는_개인기준을_방송하지_않고_검증된_실적만_싣는다():
    """[B-20 → 계약 정정(2026-07-27, 전수감사 실측)] 종전 계약은 동료 목록에 **개인 직무 기준 첫
    줄**을 실었다. 그런데 그건 시스템 불변식 '봇별 완전 격리 · 직군 공용 학습 폐지'와 정면 충돌이고,
    한 봇이 브레인 저장소에서 배운 `ops/verify.sh`가 게임 판 전원에게 퍼진 오염의 실제 통로였다
    (실측: 32명 중 4명의 첫 줄이 저장소 지식·프롬프트 자리표시자). 동료 선택에 필요한 것은 '무엇을
    실제로 해냈나'(판 중립적 실적)이지 '그 봇이 무엇을 믿나'가 아니다."""
    ...
    assert "실데이터 파이프라인" not in p                       # ① 개인 기준은 남에게 안 새어나간다
    assert "검증된 실적: 배포·인프라(DevOps) 저작 5건" in p      # ② 판 중립적 실적만 공급
