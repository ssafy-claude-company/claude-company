...
def test_골격이_제품_부품을_영역_후보로_싣는다():
    """[후보는 후보 모양으로(2026-08-07, 실측 U-536)] 08-06엔 부품을 `단위:` 줄로 깔았다 —
    '이 중 필요한 것을 고르라'고 문장으로 말하면서 형식은 이미 전부를 결정문으로 적어 둔 것이다.
    등록 파서는 `단위:` 줄을 전부 영역으로 읽으므로 고르지 않아도 이미 고른 것이 된다: U-536은
    작업 영역 12개 중 7개가 범위 항목('1개 게임 모드'·'수집물 2종'·'로컬 최고점 1개')이었고,
    그 일곱은 백로그 0으로 남았다. 부품이 **보이는 것**은 그대로 지키되(영역이 관문 모양으로
    굳는 것을 막는 08-06의 목적), 결정문 모양으로는 적지 않는다."""
    t = stage_draft_template("subtask", "안건", flow=_Flow())
    assert "적 유형 4종" in t and "사운드 이펙트" in t, "부품 후보가 아예 안 보인다"
    assert "단위: 적 유형 4종" not in t, "후보를 결정문(단위: 줄)으로 적어 두면 그게 결정이 된다"
