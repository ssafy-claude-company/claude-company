...
def test_TT_규칙3_무응찰이면_즉시_종결확인_표결():
    """[EXP-002 절제] 무응찰 → 곧장 종결 확인 표결(전원 참여 관문). '말할 게 있다'의 채널은 표결의
    [계속: N](반대=발언 의무)이다 — 그 앞에 오퍼 라운드를 더 세우는 스윕은 도입 즉시 사용자 반려로
    롤백('반대했다는 건 말할 게 있다는 것 — 이미 전원 표결이 그 자리'). 이 테스트가 그 계약을 고정."""
    st = FloorState([A, B])
    pol = TurnTakingFloor()
    t = Turn(speaker=A, passed=True)
    a = pol.resolve_open(st, t, [(B, 0)])              # 0점 응찰 = 무응찰
    assert a.kind == CLOSE_VOTE and set(a.candidates) == {A, B}   # 즉시·전원 종결 확인
