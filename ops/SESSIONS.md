...
- 로컬 PC는 Windows, 프로젝트 서버는 Linux다. 기본 진입은 murmur의 임시 웹 CLI가 아니라
  **Windows ChatGPT 데스크톱 앱의 Codex 화면 → SSH `murmur-vps` →
  `/root/ClaudeCompany`**다.
- 서버는 Codex 로그인·비대화형 PATH·영속 app-server Unix socket·GPT-5.6-Luna/max까지 준비됨.
- Windows OpenSSH 키 등록과 앱 연결 절차, 검증 및 기존 링크 퇴역 게이트는
