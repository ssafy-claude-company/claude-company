...
| **[확인]** | 독자가 **자기 자리에서** 직접 확인하는 법(URL·화면·산출물) + 결정적 증거 1~3개 — 로그 원문·수치·트랜스크립트 인용. 형용사로 대체 금지. **증거는 독자 매체로만**(화면·URL·산출물) — 개발 매체(커밋 SHA·테스트 수·파일 경로·코드 수치)는 증거로 서빙 금지, 기록 위치 한 줄로 갈음(2026-07-20 교정) | Tufte(증거는 원자료로)·Toulmin(주장 옆 증거) |
...
- **코드 라인은 사용자 검수 대상이 아니다(2026-07-08 교정: "내가 세부 코드 봐야 되냐")**: 코드의
  옳음은 세션이 보증한다 — 테스트·실기동 검증·(필요시) 교차 세션 리뷰로 끝내고, 사용자에게는
  **병합 판단 한 줄**(이 PR이 하는 일 + 병합하면 일어나는 효과)과 검증 수치만 준다. 인라인 핀은
  사용자가 판단할 방향·제품 지점(문서·화면·정책)에만 — 코드 줄에 박지 마라.
...
- **분량 상한 — 핵심 텍스트는 1페이지·2분 안에 읽힌다**(2026-07-22 도진 님 지시). 가장 중요한
  것부터 그 안에 담고, 안 들어가면 잘라서 파고들 위치만 남긴다. 길이는 성실의 증거가 아니다.
  **단 시각 자료(표·다이어그램·이미지)는 이 상한 밖 — 많아도 좋다.** 글로 풀 것을 시각으로 압축하라.
...
| 구현 세부를 사용자에게 나열(재발 — [확인]에 SHA·테스트 수·경로·수치 열거) | "난 코딩 안해. 세부적 설명 나열은 정보적으로 혼란을 줄 뿐 … worker는 너야, 그 정보를 외부로 노출하지마"(2026-07-20). 집행 정보는 커밋·STATE에만 — 보고는 독자 언어의 결과·결정·의미만 |
| 1페이지·2분 초과 장문(핵심이 파묻힘) | "가장 중요한 것 1p 이내로, 2분 안에 읽을 분량"(2026-07-22). 텍스트만의 상한 — 시각 자료(표·그림)는 많아도 좋다 |
