...
_files() { for n in "$@" test_contracts; do echo "tests/$n.py"; done; }

run_pytest() { ( cd "$R" && PYTHONPATH="$R" "$VENV/python" -m pytest "$@" -q 2>&1 | tail -1 ) || fail=1; }
...
if [ -n "$ONLY" ]; then
  ...
  case "$ONLY" in
    system) run_sysunit; run_pytest $(cd "$R" && ls tests/test_*.py | grep -vE "$(echo $GUIDE_T $ORGANT_T | tr ' ' '|')");;
    guide)  run_pytest $(_files $GUIDE_T);;
    organt) run_pytest $(_files $ORGANT_T);;
    murmur) run_sns; run_pytest tests/test_contracts.py;;
...
echo "== 3) 브레인 pytest (tests/ — 실 system/guide/organt + 계약) =="; run_pytest tests/
if [ -z "$FAST" ]; then
