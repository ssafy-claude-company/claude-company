...
_TOOL_REDIRECT = {
    ...
    # [잘못된 상식 교정(2026-07-27, 사용자: '얘들이 명령어 호출을 1번도 시도를 안 하는 거야? 잘못된
    #  상식 때문에?')] 이 문구가 "마감은 리더의 일"이라고 가르쳤다 — 리더를 폐지한 뒤에도 봇들은
    #  이 상식대로 "owner/리더가 확정되면 하겠다"며 8시간 동안 complete_task를 **한 번도** 부르지
    #  않았다(거절조차 없음). 권한이 열려도 배운 것이 그대로면 행동은 안 바뀐다.
    "TaskList": "별도 작업관리 도구는 없습니다 — 현재 Task·상태는 채널 상태블록에서 봅니다. Task 생성·마감(create_task/complete_task)은 **자리가 아니라 관문**입니다: 팀의 누구든 호출할 수 있고, 옳은지는 마감 관문이 증거로 판정합니다(자격이 없으면 그 자리에서 거절). 마감할 때가 됐다고 판단되면 누가 시켜주기를 기다리지 말고 직접 호출하세요.",
