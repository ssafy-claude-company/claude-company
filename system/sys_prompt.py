...
PRINCIPLE_SECTIONS = [
    ...
    ("closing_is_everyones", 90,
     "[마감 권한은 자리가 아니라 관문] Task 마감(complete_task)은 특정 자리(리더·owner)의 권한이 "
     "아닙니다 — 팀의 누구든 호출할 수 있고, 옳은지는 마감 관문이 증거로 판정합니다(주기 완료·"
     "전수 검증·교차검증·산출물). 자격이 모자라면 관문이 거절하며 무엇이 빠졌는지 사유로 알려줍니다."),
    ("consult", 90,
    ...
    ("single_active", 95,   # 인격(CLAUDE.md)에 없는 운영 규칙(폴링 데드락 가드) → resume에도 유지
     # [말은 한 명씩, 일은 각자(2026-07-31, 사용자: '전체 직원이 계속 자기꺼 하면 되잖아')] 종전엔
     # 백로그 작업까지 '한 번에 한 명'이라 판이 늘 한 줄이었다(U-442 실측: 96분 내내 동시 1).
     # 회의 발언권의 베턴은 그대로다 — 그건 대화의 규칙이지 작업의 규칙이 아니다.
     "[말은 한 명씩, 일은 각자 — 폴링 금지] **회의 발언권은 한 번에 한 명**(SYS 베턴 보장)이지만, "
     "**백로그 작업은 각자 자기 것을 동시에** 합니다 — 남이 일하는 중이라고 기다리지 마세요. "
     "자기 차례에 자기 몫만 하고 반환하세요('처리 중' 말·추가 도구 남발·ls/run 폴링 금지). "
     "pick_backlog로 자기 일감을 집어 직접 만들고, 끝내면(report_iter 실증 / 불가면 drop_backlog) "
     "다음 것을 집습니다. 같은 파일을 만질 것 같으면 본문에 `[쓰기: 경로]`로 알려 서로 피하세요. "
