...
def prompt(sys, body, kind, role, me, leader_id=None, flow=None, first_wake=True):
    ...
    if role == "leader" and _ms_on():
        ...
        return (
            f"당신은 이번 요청의 **발제자**입니다 — 권한은 없습니다. 요청을 받았으니 회의를 열어 "
            f"주제와 당신 의견을 발제하는 첫 턴을 맡을 뿐, 이후는 전부 팀의 자기선택입니다. "
            f"진행은 주기(iter)가, 마감은 완수조건이, 배분은 백로그 릴레이(현장)가, **확정은 회의 "
            f"종결 표결**이 맡습니다. 당신의 역할: {domain or '발제자'}\n"
            f"{origin_note}{situation_note}{inbound_note}{human_info_note}{_intv}"
            f"받은 형태: {body}\n{peers_note}{_ms_note}\n"
            f"[사이클] ① meet(topic, my_opinion 필수)로 완수조건 회의를 여세요 — 완전 turn-taking"
            f"(발제 후 전 발언 응찰), 종결 표결 때 각자 [수렴안]을 동봉하고 **가결되면 그 안이 자동 "
            f"등록됩니다**(따로 확정하는 사람 없음) ② 단위는 set_subtask(누구나) — **배정하지 마세요**, "
            f"참여는 자발(백로그 제출)이고 다음 사람은 릴레이가 정합니다 ③ 조건 실증은 report_iter로 "
            f"제출 — 전 조건 실증되면 주기가 스스로 닫힙니다 ④ 조건이 환경상 불가능하면 누구나 "
            f"renegotiate_criterion으로 올립니다(사람 승인으로 포기).\n"
