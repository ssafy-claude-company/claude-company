...
def prompt(sys, body, kind, role, me, leader_id=None, flow=None, first_wake=True):
    ...
    if role == "leader" and _ms_on():
        ...
        return (
            ...
            f"받은 형태: {body}\n{peers_note}{_ms_note}\n"
            f"[사이클] ① **meet**(topic, my_opinion 필수)로 팀 회의를 여세요 — 완전 turn-taking. 종결 "
            f"표결 때 각자 `[수렴안]`을 동봉: `단계:`(전체 로드맵, 첫 주기는 완전한 MVP) · `목표:`(이 주기) · "
            f"`조건 | 실증절차` · `단위:`(참여 도메인마다 자기 몫). **가결되면 GOAL·마일스톤·단위가 자동 "
            f"등록됩니다**(개인 set_goal/set_milestone/set_subtask 아님 — 팀 판에선 막힙니다). "
            f"② 각자 **pick_backlog**(desc='내가 할 일')로 자기 백로그를 등재 — **한 번에 한 명만** 작업하고, "
            f"끝내면(report_iter로 조건 실증 / 완수 불가면 drop_backlog로 중단) 마무리자가 남은 백로그 중 "
            f"다음 수행자를 pick_backlog(id)로 선정합니다. ③ 전 조건 실증되면 주기가 스스로 닫히고 "
            f"**사용자 보고 후 다음 단계 회의**로 갑니다. ④ 조건이 환경상 불가면 renegotiate_criterion.\n"
