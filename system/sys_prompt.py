...
PRINCIPLE_SECTIONS = [
    ("consult", 90,
     ...
     "혼자 정하지 말고 그 도메인 owner와 **먼저** 합의하세요. 되물을 상대는 직속 위임자 또는 그 도메인 owner "
     "— 더 위·멈춘 동료에겐 못 되물으니 그 산출물을 Read/Glob로 직접 확인해 검증하세요. 우리는 모두 같은 "
     "모델이라 품질은 *역할이 다른 동료와의 협의*에서만 나옵니다(혼자 일하면 같은 맹점) — 협업 빈도가 곧 품질."),
    ("single_active", 95,   # 인격(CLAUDE.md)에 없는 운영 규칙(폴링 데드락 가드) → resume에도 유지
     "[단일활성 — 요청은 하나씩, 폴링 금지] 동료는 **한 번에 한 명만** 일합니다(SYS 베턴 보장) — 백그라운드·"
     "시차 도착·동시 진행은 없습니다. request는 한 턴에 하나만 보내고, '[위임됨]'이 즉시 돌아오면 그 턴을 "
     "마치세요('처리 중' 말·추가 도구 호출·재위임·ls/run 폴링 금지). SYS가 그 동료를 끝까지 완주시켜 결과와 "
     "함께 당신을 다시 깨웁니다 — 그때 검증·통합하고 다음 요청을 보냅니다. 받은 산출물이 미완('⚠ 턴 한도 "
     "도달')이면 그 owner에게 request(Work)로 '이어서'를 보내야 계속됩니다."),
    ("verify", 90,
...
def craft_note(sys, me, first_wake=True) -> str:
    """[정체성·[경험] = first_wake 각인, 되살리기 없음] 직무·개인 기준(정체성, 불변)과 [경험] 요청은 첫 wake에
    1회만 가르친다 — resume엔 재주입 안 함. [경험]의 내구 홈은 **report 툴의 experience 필드**(구조적 캡처),
    정체성은 대화 기억 + role_profiles. 미작성 직무기준 요청도 first_wake에 한 번(써두면 영속).
    ...
    notes, missing = [], []
    for j in jobs:
        p = sys.role_profiles.get(j)
        if p:
            # [직군 공용 베이스라인] 직무 기준은 누적 표준 — 신규 봇도 첫 작업부터 부트스트랩(F). 단 불변이라
            # 재-wake엔 대화에 이미 있음 → 첫 wake만 주입(전에 준 걸 또 주지 않는다).
            if first_wake:
    ...
    # [B-19] 수면 증류된 '개인 기준'이 있으면 직군 기준 다음에 그것을 주입하고 원시 경험 줄들을
    # *대체*한다(≤600자 — 원시 6줄 대비 자수 비증가·토큰 중립). 증류 전이면 종전대로 원시 주입
    # (원석 풀은 계속 쌓여 8건 도달 시 distill_bot이 다시 압축한다 — 개인 플라이휠).
    mine = (sys.bot_profiles.get(me) or "").strip()
    exp = sys.bot_experience.get(me)
    if first_wake:      # 개인 기준·원시 경험도 불변이라 재-wake엔 대화에 이미 있음 → 첫 wake만 주입
        if mine:
            notes.append("[당신의 개인 기준 — 당신 자신의 경험이 수면 증류로 압축된, 직군 공용 위의 "
                         "당신만의 층. 이 기준이 당신을 같은 직군 동료와 다르게 만듭니다]\n" + mine)
        elif exp:
            notes.append("[당신의 최근 경험 — 당신이 실제 작업에서 직접 얻은 교훈. 같은 함정을 반복하지 마세요]\n"
                         + "\n".join(f"- {e}" for e in exp[-6:]))
    if jobs and first_wake:   # [경험] 요청 = 첫 wake 1회 각인(내구 홈은 report 툴 experience 필드)
    ...
    if missing and first_wake:   # 미작성 직무기준 작성 요청은 first_wake에 한 번(써두면 role_profiles에 영속)
        notes.append(
...
def prompt(sys, body, kind, role, me, leader_id=None, flow=None, first_wake=True):
    ...
    return (
        ...
        + ((
            "[운영 규칙 — 원칙은 CLAUDE.md 인격에 있고 여기선 이 협업 기계의 규칙만] 협의(Info) 중엔 제안·합의만 하고 "
            "**파일 Write는 구조적으로 차단**됩니다(구현은 Work로 위임받은 뒤). owner Work를 미완으로 끝냈으면(턴 한도) "
            "'어디까지·뭐가 남았는지'만 정확히 적어 반환하세요 — SYS가 그 지점부터 '이어서' 잇습니다('진행 중·마저 "
            "하겠다'로 멈추지 말 것).\n"
            "[이니셔티브의 방향] 도메인 *밖*의 것(남이 고칠 결함·타 도메인 구현·재배정)은 직접 행동 말고 **보고/제안**으로 "
            "올리세요 — 교차도메인 일은 시스템이 리더 조율로 배정합니다."
        ) if first_wake else "")
    )
