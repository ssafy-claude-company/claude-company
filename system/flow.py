...
class Flow:
    ...
    def __init__(self, guide, channel_id, guild_id, leader_id, bot_info=None):
        ...
        self.floor_bidders = set()     # [자기 등재 판의 '기회' 원장(2026-08-04, 사용자: '위임이란건 존재하지
                                       #   않아')] 이 흐름에서 발언권 응찰 프롬프트를 한 번이라도 받은 사람.
                                       #   자기 등재·자기선택 판에서 일감을 등재할 유일한 통로가 발언권이라,
                                       #   '이 사람에게 기회가 갔는가'는 여기서만 사실로 확인된다(기여 관문 입력).
