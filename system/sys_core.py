...
class Sys:
    ...
    async def _drive_task_close(self, flow):
        ...
        # [흡수는 우회하지 않는다(2026-08-04, 사용자: '우회하면 안되지 근본적인 해결방안으로')]
        # 한때 여기서 SYS가 유휴 멤버를 골라 대신 Work를 보냈다. 그건 두 가지로 틀렸다 —
        # ① 흡수의 원인이 내 정리 로직이 팀의 등재를 지운 것이었으므로 증상 우회였고,
        # ② "시스템이 담당을 골라 위임"은 2026-07-20에 폐지된 개념이다(자기 등재·자기선택이 원칙).
        # 원인을 고친 지금은 각자 올린 몫이 남아 있으므로 본인이 집어서 한다.
