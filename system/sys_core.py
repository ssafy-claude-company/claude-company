...
class Sys:
    ...
    async def _drive_task_boundary_e2e(self, flow):
        ...
        # 세그먼트 수로 세면 같은 순회에서 연속 호출돼 순식간에 소진된다(실측: 같은 초에 2회) —
        # **시간**으로 쥔다. 이 동안은 회의·작업이 돌고, 지나면 다시 열어 본다.
        _until = float(getattr(flow, "_e2e_ratify_until", 0) or 0)
    ...
    async def _drive_task_close(self, flow):
        ...
        # [흡수 차단도 SYS가 푼다(2026-08-04, 실측 U-478)] 마감 게이트는 "회의에서 말만 하고 Work를
        # 한 번도 못 받은 멤버"가 있으면 닫지 못하게 하고(흡수 차단), 그 사람에게 실제로 위임하라고
        # 한다. 그런데 그 위임을 보내는 장치가 없다 — 교차검증만 SYS가 대신 보낸다.
        ...
        if _todo:
            try:
                ...
                await _tls2["request"].handler({
                    ...
                    "different_deliverable": "이 직군의 도메인 몫 — 제작자가 대신 쓴 것이 아니라 본인 작업",
                    "body": ("[SYS — 마감 전 도메인 기여] 이 Task는 산출물과 전수 검증이 끝났는데, "
                             "**당신의 직군 도메인이 작품에 반영되지 않았습니다**(이 흐름에서 "
                             "Write/Edit/run 0회 — 회의 발언만). 마감 관문이 그 사실로 막고 있습니다.\n"
                             "**당신이 직접** 작업공간을 보고, 당신 도메인에서 지금 산출물에 빠졌거나 "
                             "부족한 것을 **한 가지 이상 실제로 고치거나 더하세요**(파일 편집 또는 "
                             "검증 실행). 무엇을 왜 했는지 한 줄로 보고하면 됩니다.\n"
                             "정말 이 작품에 당신 도메인이 불필요하다면 그 이유를 한 줄로 적어 주세요 — "
                             "그 판단도 당신 몫입니다(다른 사람이 대신 넘길 수 없습니다)."),
                })
            except Exception:
                pass
            ...
            try:
                ...
                if _peer:
                    ...
                    await _tls["request"].handler({
                        ...
                        # [위임 경로를 문구로 닫는다(2026-08-03, 실측 U-478)] 지목된 동료가 이
                        # 요청을 받고 한 일이 **다시 위임**이었다("독립 QA 최종 인수검증 위임을
                        # 유지합니다" 14:52·15:14). 위임 응답은 교차검증으로 세지 않으므로
                        # (카운터는 이 request의 응답에서만 오른다) 관문은 그대로 막히고, 리더는
                        # 마감만 반복 호출하다 판이 20분 무진행으로 멎었다. 받은 사람이 직접
                        # 해야 한다는 사실과 그 이유를 본문에 명시한다.
                        "body": ("[SYS — 마감 전 독립 검증] 이 Task의 산출물이 완성되고 전수 검증도 "
                                 "통과했습니다. 마감에는 제작자가 아닌 동료의 실제 사용 검증 1회가 "
                                 "필요합니다.\n**이 검증은 당신이 직접 해야 합니다** — 다른 사람에게 "
                                 "넘기면 관문은 열리지 않습니다(위임했다는 응답은 검증으로 세지 "
                                 "않습니다. 지금 이 요청에 대한 **당신의 실사용 보고**만 셉니다).\n"
                                 "산출물을 **사용자처럼 처음부터 끝까지 직접 써보고**(화면이면 "
            ...
            if _mode == "e2e":
                ...
                if _opts is not None:
                    try:
                        ...
                        pass
            if _mode == "close":
                # [마감 전용 표면(2026-07-27, U-067)] 네이티브 편집 표면을 빼고 '보고 닫는' 것만 남긴다 —
                # 마감 관문 앞에서 새 작업을 벌이거나 논의로 새지 않게(e2e 전용 턴과 같은 방식).
                organt._codex_read_only = True
                _opts_c = getattr(organt, "options", None)
                if _opts_c is not None:
                    try:
                        organt.options = dataclasses.replace(
                            _opts_c,
                            allowed_tools=[
