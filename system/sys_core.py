...
class Sys:
    ...
    async def _drive_task_close(self, flow):
        ...
        await self.run_turn(
            flow, actor,
            "[SYS — Task 마감 전용 턴] **이 판은 지금 재개되어 돌아가는 중입니다.** 채널에 남아 있는 "
            "'재개를 누르면 진행해요' 안내는 지나간 기록입니다 — 사람의 재개를 기다리지 마세요. "
            "지금 이 턴에서 바로 호출할 수 있고, 지금이 그 시점입니다.\n"
            "전 주기가 닫혔고 Task 경계 전수 검증(e2e)이 통과했습니다. "
        ...
        if turn_no >= 2:
            try:
                ...
                if _why:
                    await self.run_turn(
                        flow, actor,
                        "[SYS — 마감 관문이 요구하는 검증 턴] **이 판은 지금 돌아가는 중입니다** "
                        "('재개' 안내는 지나간 기록 — 기다리지 마세요). 마감을 시도했더니 관문이 아래 사유로 "
