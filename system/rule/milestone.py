...
_STAGE_META = {
    ...
    "milestone": ("이번에 **완성해서 사용자에게 보여줄 딱 하나**를 정한다(전체 말고 이번 것)",
             ...
             "★**첫 주기(최소버전)는 사용자가 브라우저에서 실제로 조작해볼 수 있는 '도는 것'이어야 합니다** "
             "— 기획서·규격 문서·페이퍼 검증 같은 산출물을 첫 주기로 삼지 마세요(그건 이 주기 안의 준비 "
             "작업이지 완성 단위가 아닙니다). 완수조건도 'run/브라우저로 동작 확인'이 되는 실행물 기준으로 "
             "잡으세요. 문서만 나오는 주기는 사용자가 체감할 게 없습니다(U-036 실측: 첫 주기가 '페이퍼 "
             "검증'이라 예산 절반을 문서에 쓰고 게임은 안 돌았습니다)."),
...
def stage_draft_template(stage, agenda=""):
    ...
    body = {
        ...
        "milestone": ("단계: <전체 로드맵 — 예: 최소버전 → 확장>\n"
                      "이번 주기: <이번에 완성해 사용자에게 보여줄 딱 하나 — 브라우저에서 실제로 도는 것>\n\n완수조건:\n"
                      "(주의 ①**'이번 주기' 범위의 조건만** — 뒤 단계 몫(모션 세부·디자인 토큰 등 완제품 "
                      "사양)을 넣으면 이번 주기가 영영 안 끝납니다. 그건 그 단계 주기에서.\n"
                      " ②**첫 주기는 실행물** — 기획서·규격·페이퍼 검증이 아니라 run/브라우저로 동작 "
                      "확인되는 '도는 것'. 문서는 그 안의 준비 작업이지 완성 단위가 아닙니다.)\n"
                      "- <조건> | 실증: <run/브라우저로 확인하는 절차>\n"),
