...
def cycle_delivery_error(flow) -> str:
    ...
    # [밖에 올렸어도 우리 판에서 열려야 검증이다(2026-07-31, 사용자: '우리 주소가 아니여도 저렇게
    # 다르게 배포했으면 인정이긴 한데 우리 서비스에도 배포 해둬야 검증을 하겠지?')] 외부 배포는 배포로
    # 인정한다 — 다만 그것만으로 주기를 닫지는 않는다. 실측(U-442)에서 외부 주소는 401(Sign in
    # required)이었고, 사용자는 결국 아무것도 열지 못했다. 우리 판이 직접 서빙하는 입구(앱 풀·정적
    # 진입)가 있어야 사람이 눌러서 확인할 수 있다.
    ...
    if _url:
        return (f"주기 완수 보류: 밖({_url})에는 올렸지만 **이 판에서 열 수 있는 입구가 없습니다** — "
                f"사용자는 우리 화면의 '완성작'에서 눌러 확인합니다(외부 주소는 로그인 벽 뒤이거나 "
                f"사라질 수 있습니다).\n지금 할 일 하나: `deploy` 도구로 **이 판의 앱 풀에도** 올리세요 "
                f"— 인증 없는 공개 주소가 곧바로 생깁니다. 올린 뒤 다시 선언하세요(외부 배포는 그대로 두면 됩니다).")
...
_STAGE_META = {
    "goal": ("이 Task로 **무엇을 만들지**를 정한다",
             ...
             "내용 폭: ⟦'알차다'의 하한을 수치로 + **깊이 축 최소 1개**(성장·해금·조합·강화·메타 재화·난이도 곡선) — 예: 게임이면 '요소 4종 · 강화 3택1', 도구면 '기능 6종 · 자동화 4단', 문서면 '章 8개 · 심화 3단' — 형태는 당신들이 정합니다 / "
             ...
             # 판은 언제나 가장 좁은 실증 가능 범위로 흘러간다. 넓이는 기획의 결정 사항이고, 결정은
             # 수치여야 다음 회의(완수 기준)가 조건으로 잠글 수 있다. 시스템은 숫자의 크기를 판단하지
             # 않는다 — 결정이 있었는지만 본다.
             ...
             # [요구를 채우면 요구만큼만 나온다(2026-08-05, 사용자: '몹을 추가하라는 명령에
             # 방패병이라는 얘를 두고 앞으로 오는 공격은 데미지가 감소하는등의 고도화 된 봇의
             # 창의적 설계')] 내용 폭은 '얼마나 많이'를 정할 뿐 '요구 밖'을 정하지 않는다.
             # 실측 U-496: 게임 코드 1,956줄에 검증 코드 9,655줄, 요구 밖 기여 0건 — 968턴
             # $166.70이 명세 충족과 그 실증에만 쓰였다. 얹을 자리를 회의가 만들지 않으면
             # 아무도 얹지 않는다. 시스템은 기여의 좋고 나쁨을 판단하지 않는다(있었는지만).
             "**창의 설계는 요구를 그대로 만족시키는 것 위에 하나 더 얹는 결정입니다** — 요구 "
             "명세만 채우면 요구만큼만 나옵니다. '몹을 추가하라'는 요구에 방패병(앞 열 피해 "
             "감소)을 얹은 것이 그 예입니다. 지금 정한 기여는 완수 기준 회의가 조건으로 잠급니다.\n"
    ...
    "criteria": ("**무엇이 되면 이 Task가 끝인가**를 정한다(만들 것은 이미 정해짐)",
             ...
             # [검증이 제품을 넘어서면 크레딧은 실증에만 쓰인다(2026-08-05 실측)] U-496은 조건마다
             # 새 검증기를 써서 verify_ui.py 9,655줄 · 브라우저 검증 1,031줄 · 테스트 3,214줄을
             # 남겼는데 제품(게임)은 3,285줄이다(4.2:1). U-506/U-507도 검증이 제품과 맞먹는다.
             # 진입점 하나로 모으면 같은 관문을 훨씬 적은 턴으로 통과한다 — 강제가 아니라 코칭.
             "**실증 명령은 판에 이미 있는 검증 진입점 하나로 모으세요**(예: `npm run verify` 하나에 "
             "케이스를 더하는 방식) — 조건마다 새 검증기를 쓰면 검증 코드가 제품을 넘어섭니다"
             "(실측: 게임 3,285줄 대 검증 13,900줄).\n"),
...
def stage_draft_template(stage, agenda="", flow=None):
    ...
    [내용 폭 → 완수조건 승계(2026-08-04)] goal 회의가 정한 '내용 폭' 항목들은 완수 기준 회의의
    조건 초안으로 미리 실린다 — 자리표시 가드가 각 항목의 실증을 채우게 강제하고, 채워진 조건은
    GOAL 잠금이 되어 기존 하드 게이트(최종 주기 실증·release verifier)가 그대로 지킨다. 넓이의
    결정이 텍스트 선언에서 끝나지 않고 검증 분모에 도달하는 유일한 통로다."""
    _floor_lines = ""
    try:
        _fl = list(getattr(getattr(flow, "current", None), "content_floor", None) or [])
        if stage == "criteria" and _fl:
            _floor_lines = "".join(
                f"- 내용 폭 — {it} | 실증: ⟦측정 가능한 검사 또는 exact command⟧\n" for it in _fl[:12])
        # [창의 설계도 조건이 되어야 살아남는다(2026-08-05)] 내용 폭과 같은 통로 — 선언에서
        # 끝나면 마지막 주기의 실증 분모에 닿지 못하고, 닿지 못한 것은 만들어지지 않는다.
        _cv = list(getattr(getattr(flow, "current", None), "creative", None) or [])
        if stage == "criteria" and _cv:
            _floor_lines += "".join(
                f"- 창의 설계 — {it} | 실증: ⟦측정 가능한 검사 또는 exact command⟧\n" for it in _cv[:6])
    except Exception:
        _floor_lines = ""
    body = {
        "criteria": ("완수조건:\n" + _floor_lines
...
def register_stage(flow, stage, prop, origin=""):
    ...
    if stage == "goal":
        ...
        _DEPTH_RE = _re.compile(r"(성장|레벨업|레벨 업|업그레이드|강화|증강|해금|언락|"
                                r"스킬|특성|퍽|시너지|조합|빌드|덱|장비|아이템 획득|"
                                r"메타|영구|누적|진척|진행도|난이도 곡선|웨이브|스테이지 진행|"
                                r"보스|엔딩|스토리|맵 확장|모드 해금)", _re.I)
        if not _DEPTH_RE.search(_floor_raw):
            return False, ("'내용 폭:'이 **가짓수**만 셉니다 — 판이 진행되며 달라지는 **깊이 축**이 "
