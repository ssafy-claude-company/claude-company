...
# [개념 폐지 — 레거시 방어 가드만(2026-07-08, 사용자: '예비라는건 없어 이제')] '예비'(채용 대기 인력)
# 개념은 폐지됐다 — recruit는 그 직군 전문가를 즉석 생성(genesis)한다. 이 라벨·판별은 새 의미를 갖지
# 않으며, 남은 역할은 단 하나: 레거시 데이터(옛 로스터/Discord 역할)에 '예비'/무직 라벨이 남아있을 때
# 그런 봇이 팀·회의·표결·위임에 섞여 들어가는 것을 막는 방어. 새 코드는 이 개념을 참조하지 말 것.
_SPARE_LABEL = "예비"


def _is_spare(flow, oid) -> bool:
    """레거시 방어: 직군 미배정(옛 '예비' 라벨) 봇 판별 — 개념 자체는 폐지(위 주석).

    [채용봇은 시스템 존재(2026-08-04, 사용자: '채용 봇은 Organt가 아니고 시스템적인 존재야')]
    채용 역할은 신입을 빚는 제네시스의 리크루터일 뿐 판의 구성원이 아니다 — 그런데 실측 U-504:
    채용 봇이 팀 채용 한 번 없이 **백로그 75건 전부를 혼자 등재·수행**하며 판을 뛰었다(활동 322줄).
