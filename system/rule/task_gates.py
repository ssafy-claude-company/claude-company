    ...
    if has_product and not flow.current.contrib_checked:
        ...
        if contrib_idle and (not _cd or _absorbed):
            ...
            absorbed_note = (
                ...
                f"그러니 이들이 등재할 자리를 여세요(①): 마감을 멈추고 회의에서 그 도메인의 단위를 열면 "
            ...
            return (
                ...
                f"**작품에 반영되지 않았습니다**. 셋 중 하나를 택하세요: ① 필요한 도메인이면 마감을 멈추고 "
                f"회의에서 그 단위를 열어(팀 판의 단위는 회의 수렴안으로 등록됩니다) 본인이 등재·수행하게 "
                f"하고 그 산출물을 교차 검증까지 받으세요 ② 애초에 불필요했으면 "
