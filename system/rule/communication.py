        ...
        async def _ratify_vote(prop):
            ...
            def _rbody(c):
                ...
                return (f"[회의 — 결론 확정 표결] 주제: {topic}{_fr}\n제출된 결론:\n{prop}\n\n"
                        ...
                        f"이 안건의 결론으로 충분하면 `[찬성]`, 이 안건 범위에서 빠진 게 있으면 "
                        f"`[반대: 무엇이 빠졌는지 한 줄]` — **딱 이 형식으로 한 줄만** 답하세요. 마커를 별표(*)로 "
                        f"감싸거나, `[찬성]`·`[반대]` 외 다른 대괄호·'근거:'·'판단:' 라벨·이 지시문 인용을 붙이지 "
                        f"마세요(사유만 자연스러운 한 줄). 도구 호출·파일 확인 금지 — 지금 이 텍스트만 보고 즉답. "
                        f"전원 찬성이어야 확정, 반대는 병합 후 재표결. "
            ...
            for m, res, note in await _fork_collect(flow, me_id, _voters, _rbody, micro=True):
                ...
                # [사유 정규화(2026-07-22, 사용자 U-044: '**[찬성]**·**근거**·프롬프트 인용이 사유에 그대로
                # 노출')] 봇이 마커를 별표로 감싸거나(**[찬성]**)·역할 라벨([데이터 엔지니어 판단])·'근거:'·
                # 이 지시문('당신(…)의 판단:')을 사유에 섞어 쓴다 — 표시 전 정제해 실질 사유 한 줄만 남긴다.
                _rn = t.replace("*", "").strip()                          # 마크다운 굵게/이탤릭 제거
                # [진짜 사유 우선(2026-07-22, 사용자 U-044: 송지안 '읽겠습니다, 지금 이 텍스트 기반으로'가
                # 반대 사유로 뜸)] 봇이 잡소리 서두(프롬프트 흉내) 뒤에 [반대: 진짜사유]를 다는 경우 —
                # 첫 줄만 보면 서두를 집는다. 전체에서 [반대: X]를 먼저 찾아 진짜 사유를 쓴다.
                _mn = re.search(r"\[\s*반대\s*[:：]\s*([^\]\n]{2,})", _rn)
                if _mn:                                                    # [반대: X] → X(위치 무관)
                    _reason = _mn.group(1).strip("[]").strip()
                    ...
                    _reason = _cl or next((x.strip().strip("[]").strip()   # 첫 줄이 마커뿐이면 다음 실질 줄
