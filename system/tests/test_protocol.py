"""재구현 ① 검증: 구조화 메시지 프로토콜(docs Discord.md 포맷) — PJT tests/test_protocol.py의 unittest 포팅.
...
class ProtocolFormatTest(unittest.TestCase):

    def test_request_포맷_docs형식(self):
        s = format_request(B, Kind.WORK, "보고서 완성")
        assert s.splitlines() == ["[Request]", f"To: <@{B}>", "Kind: Work", "Body: 보고서 완성"]

    def test_response_포맷_docs형식(self):
        assert format_response("완료") == "[Response]\nBody: 완료"

    def test_task_status_블록_docs형식(self):
    ...
    def test_task_status_result_종료시(self):
        ts = TaskStatus(task_id="1", result="완수")
        assert "- result: 완수" in format_task_status(ts)


class ProtocolParseRoundtripTest(unittest.TestCase):

    def test_request_왕복(self):
