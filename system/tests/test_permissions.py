"""기능8 검증: PreToolUse 권한 차단 훅 — PJT tests/test_permissions.py의 unittest 포팅.
...
class PreToolUseHookTest(unittest.TestCase):
    ...
    def test_리더_origin_Work_구현_허용(self):
    ...
    def test_리더는_위임된_owner도메인_대리구현_차단(self):
    ...
    def test_위임전_owner0_Task는_리더구현_허용(self):
    ...
    def test_개입은_목표확정_전_수정차단(self):
        """개입(기존 프로젝트 수정)에서 Task Goal이 없으면 Write/Edit 거부 — 재현·목표합의 전
        즉흥수정 차단. Goal이 확정되면 허용 → '목표 먼저, 그다음 수정' 순서를 구조적으로 강제."""
        a = FakeAudit()
    ...
    def test_리더_위임없이_단독구현_차단(self):
        """팀이 있는 Task에서 리더가 Work 위임 없이 혼자 다 쓰는 걸 차단 — '자문(Info)만 받고 독식'
        구멍을 막는다. 한 파일은 grace, 그다음부턴 위임해야 함. 위임하면 다시 허용. 단독 Task는 무제한."""
        a = FakeAudit()
    ...
    def test_개입아닌_새작업은_목표게이트_미적용(self):
    ...
    def test_개입_리더_단독run_독식_차단(self):
        """[근본] 개입에서 리더가 'Task도 안 열고' 또는 '위임 없이 run만 반복'하면 차단 —
        create_task+위임 후엔 풀린다(리더의 최종 검증 run 허용)."""
        a = FakeAudit()
    ...
    def test_쓰기리스_샌드박스밖_차단_안은_허용(self):
        """[경쟁 구현 — 쓰기 리스] 리스가 배정된 행위자는 자기 샌드박스 안에만 쓴다 — 파일 충돌이
        구조적으로 불가능. 리스 없는 행위자는 종전대로."""
        a = FakeAudit()
        flow = _FakeFlow2(_comm_with((0, 11, Kind.WORK), (11, 12, Kind.WORK)),
    ...
    def test_fork가지_Info수집은_선구현차단_Work가지는_허용(self):
        """fork 수집 가지는 comm 프레임을 열지 않으므로 flow.fork_kind가 선구현 게이트를 잇는다 —
        표결·회의 1라운드(Info 가지)의 Write는 차단, 경쟁 구현(Work 가지)은 허용."""
        a = FakeAudit()
    ...
    def test_리더_흡수_상대적_차단_팀합_초과(self):
        """[리더 흡수 차단 #8] 코디네이터의 직접 doing(act_by 리더)이 '팀 전체 합'을 넘으면 흡수 —
        grace(>=8) 이후 Write/Edit/run 차단. 위임으로 팀 활동이 오르면(자가치유) 다시 풀린다."""
        a = FakeAudit()
    ...
    def test_리더_흡수_차단_교착방지_도달동료없으면_통과(self):
        """[교착 방지] 도달 가능한 동료가 없으면(솔로 또는 전원 예비) 흡수 차단이 풀린다 —
        차단은 '맡길 사람이 실제로 있을 때'만(안정성 우선)."""
        a = FakeAudit()
        run_allowed = ALLOWED + ["mcp__guide__run"]
    ...
    def test_소유경계_타직군_소유파일_편집_차단(self):
        """[소유-기반 경계] 다른 직군이 *만든* 파일(file_owner)을 편집하려 하면 차단 —
        QA(12)가 백엔드 소유 server.js를 Edit 시도 → 거부(보고/요청 유도)."""
        a = FakeAudit()
    ...
    def test_소유경계_자기직군_소유파일_편집_허용(self):
    ...
    def test_소유경계_미소유_새파일_허용_및_생성직군_귀속(self):
        """미소유(새) 파일은 통과 + 작성 직군으로 귀속(첫 작성=생성). 통과·기록·영속."""
        flow = _FakeFlow2(_comm_with((0, 11, Kind.WORK), (11, 12, Kind.WORK)), current=_FakeTask(owner=12), leader=11)
    ...
    def test_소유경계_리더도_타도메인_차단_S2(self):
        """[S2 협업재설계 — 게이트4] 리더도 *타 도메인 owner 파일*은 편집 차단(owner에게 위임).
        단 자기 도메인·미소유(통합) 파일은 자유."""
        flow = _FakeFlow2(_comm_with((0, 11, Kind.WORK)), current=_FakeTask(owner=0), leader=11)
    ...
    def test_흡수차단_같은도메인_idle은_차단안함_그리고_도달불가_통과(self):
        """[게이트 #9 정밀성·교착방지] (a) 같은 도메인 idle 동료엔 발동 안 함.
        (b) distinct 전문가라도 예비/도달불가면 통과."""
    ...
    def test_막힘흡수차단_막힌동료_일_대신못함_재요청유도(self):
        """[게이트 #10] 하위 담당이 막혀 베턴이 위임자에게 돌아온 순간(flow._stall_victim 기록),
        위임자가 막힌 사람과 도메인이 다른 일을 대신 Write하면 차단 — '같은 사람 재요청' 유도.
        막힌 사람이 다시 act하면 해제."""
        a = FakeAudit()
        ...
        out = _run(hook, "Write", {"file_path": "server.js"})            # 백엔드 일 대신 작성 시도
        assert out["hookSpecificOutput"]["permissionDecision"] == "deny"
    ...
    def test_막힘흡수차단_같은도메인_허용_그리고_무응답_폴백(self):
        """[게이트 #10 정밀성·교착방지] (a) 같은 도메인이면 차단 안 함(같은 분야 이어받기).
        (b) 막힌 사람이 끝내 무응답이면 N회 차단 후 폴백(통과)."""
    ...
    def test_쓰기리스_다중경로_병렬Work(self):
        """[RFC-006 parallel_work] 가지마다 '파일 목록' 리스 — 목록 중 어느 하나면 허용, 전부 밖이면 차단."""
        a = FakeAudit()
